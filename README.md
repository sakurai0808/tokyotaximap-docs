...
<br>
PC でも検索を常設する。しかし広くスペースがある PC ではピンマップの利用がより多いと考えられ、モバイルのように「第一導線:検索 第二導線:ピンマップ」という優先順位の意識はしない。一般的なアプリケーションの UI として検索はどこからでもできるようにし、ピンマップはトップページのみに配置する。
...
この機能は、モバイルでのハンバーガーメニュー操作時に動作するものとする。理由としては良いアイデアではあるものの、「QR コードを用いて Web ページを共有する」というのは一般的ではなく、表示の優先度合いは低いと考えられるためである。
