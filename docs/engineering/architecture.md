...
当面はエンジニア 1 人が記事を管理するため、検索・ピンマップ・AI リライトなどプロダクト要件の開発を優先する。将来、非エンジニアの編集者が必要になった段階では、Git ベース CMS（Decap CMS / TinaCMS 等）の導入を検討する。記事の実体は Markdown のまま維持する。
...
ビルド時に front matter を検証する。`category`・`city`・`tags` は enum（許可値リスト）で制約する。
