...
- カテゴリ別フォルダは使わない（URL は `/{slug}/`。カテゴリは front matter）

設計サンプル: [ザ・キャピトルホテル東急](../design/samples/articles/the-capitol-hotel-tokyu.md)
...
施設サンプル 1 本の設計とファイル配置が固まった。続けて次を行う。

- 可能なら `pinmap: false` や別カテゴリの 2 本目で条件付きキーの差を確認する
- Next.js での Markdown 読み込み、続けて Pagefind PoC
